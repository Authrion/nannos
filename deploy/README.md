...
- [x] Порты 80 и 443 открыты (`sg-0f6e9018a1cd2dd16`), проверено снаружи
- [x] Docker установлен (29.1.3), каталог /opt/nannos создан
- [x] Ключ для GitHub Actions добавлен (`~/.ssh/nannos_deploy`)
- [x] Схемы в базе Neon созданы (`nannos`, `console`, `keycloak`, владелец `neondb_owner`)
- [x] Файлы в `deploy/` и `.github/workflows/` созданы, YAML проверен
- [ ] Секреты в GitHub
- [ ] Первая сборка
- [ ] Первый деплой
- [ ] Вход в UI работает
